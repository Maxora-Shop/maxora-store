...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  if (req.method === 'GET') {
    ...
    if (memorySettingsCache && now - lastFetchTime < 30000) {
      res.setHeader('Content-Type', 'application/json');
      res.setHeader('Cache-Control', 'public, s-maxage=30, stale-while-revalidate=120');
      res.statusCode = 200;
      res.end(JSON.stringify({ success: true, settings: memorySettingsCache, cached: true }));
      return;
    }
    ...
    try {
      ...
      console.warn('Firestore settings read error (fallback to memory cache):', e?.message || e);
    }
