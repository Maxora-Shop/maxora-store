...
export const HotDealsSection: React.FC<HotDealsSectionProps> = ({
  ...
  // Filter for products marked as is_hot_deal by Admin, or fallback to products with badge HOT or highest discount
  const hotDealsProducts = React.useMemo(() => {
    const explicitlyMarked = products.filter(
      (p) =>
        (p.is_hot_deal === true || p.is_hot_deal === 1 || String(p.is_hot_deal) === '1') &&
        p.active !== 0 &&
        p.active !== false
    );
    ...
    // Fallback to products with badge 'HOT' or highest discounts
    const hotBadge = products.filter(
      (p) => p.active !== 0 && p.active !== false && (p.badge === 'HOT' || p.badge === 'SALE')
    );
    if (hotBadge.length > 0) return hotBadge.slice(0, 4);
  ...
  if (hotDealsProducts.length === 0) return null;
    ...
    <section className="my-6 sm:my-10 w-full">
      ...
      <div className="flex items-end justify-between mb-4 sm:mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-rose-500/10 text-rose-600 flex items-center justify-center font-black shrink-0 border border-rose-200">
            <Flame className="w-5 h-5 sm:w-6 sm:h-6 fill-rose-500 text-rose-600" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="bg-rose-100 text-rose-800 font-extrabold text-[10px] sm:text-xs px-2 py-0.5 rounded-md uppercase tracking-wider">
                Special Offers
              </span>
              <span className="text-zinc-500 text-xs font-medium hidden sm:inline">
                Handpicked hot discount deals
              </span>
            </div>
            ...
            >
              ...
              <div className="relative aspect-square bg-zinc-50 border-b border-zinc-100 flex items-center justify-center p-3 sm:p-4 overflow-hidden">
                ...
                <div className="absolute top-2.5 left-2.5 flex flex-col gap-1 z-10 pointer-events-none">
                  ...
                  {discountPercent > 0 && (
                    <span className="bg-zinc-950 text-white font-black text-[9px] sm:text-[10px] px-2 py-0.5 rounded-md shadow-xs">
              ...
              <div className="p-3 sm:p-4 flex-1 flex flex-col justify-between">
                <div>
                  <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-1 truncate">
                    {product.brand || product.category || 'Maxora BD'}
                  </div>
                  ...
                  <div className="flex items-center gap-1 mb-2">
                    ...
                    <span className="text-[10px] text-zinc-400">
                      ({ratingStats && ratingStats.count > 0 ? ratingStats.count : '18+'})
                    </span>
                  </div>
                </div>
